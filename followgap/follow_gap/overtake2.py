...
class OvertakeFollowGap(Node):
    def __init__(self):
        ...
        self.get_logger().info("OvertakeFollowGap (SAFETY UPGRADED) started.")
    ...
    def opponents_callback(self, msg: PoseArray):
        # placeholder constant velocity assumption
        self.opponents = [TargetVehicle(p, 5.0) for p in msg.poses]
    ...
    def lidar_callback(self, scan: LaserScan):
        ...
        # ---------- gap follower core ----------
        closest_idx = int(np.argmin(proc)) if proc.size else 0
        ...
        # opponent repulsion (safety upgrade)
    ...
    # ------------------------------------------------------------
    # Target selection with *stability* (prevents glitch run-overs)
    # ------------------------------------------------------------
    def select_front_target_stable(self) -> Optional[TargetVehicle]:
        now = self.now_sec()

        # pick opponent closest in front region
    ...
    def select_front_target(self) -> Optional[TargetVehicle]:
        ...
        for o in self.opponents:
            ...
            # longitudinal and lateral relative position
            s = math.cos(yaw)*dx + math.sin(yaw)*dy   # forward
            l = -math.sin(yaw)*dx + math.cos(yaw)*dy  # lateral

            if s > 0 and s < best_s and abs(l) < 3.0:  # must be near front corridor
                best = o
                best_s = s
        return best
    ...
    def step_fsm(self, target, predicted):
        ...
        # ---------- OVERTAKE ----------
            ...
            # must be physically past AND have clear forward wedge
        ...
        # ---------- RETURN ----------
            ...
            # apply minimum return horizon to avoid cut-off
    ...
    def apply_opponent_repulsion(self, idx: int, length: int, target: Optional[TargetVehicle]) -> int:
        ...
        # if opponent is near our left -> steer slightly right
        if lat > 0:
    ...
    def preprocess_lidar(self, ranges: np.ndarray) -> np.ndarray:
        ...
        # 270° FOV → crop to forward region
        if n > 270:
    ...
    def side_repulsion_shift(self, proc: np.ndarray) -> int:
        ...
        diff = right_avg - left_avg     # + → obstacle on left → steer right
    ...
    # ------------------------------------------------------------
    # Speed policy (Option 2: safer adjacent-to-opponent behavior)
    # ------------------------------------------------------------
    ...
    def clear_on_pass_side(self) -> bool:
        ...
        min_val = float(np.min(self.proc_latest[lo:hi]))
